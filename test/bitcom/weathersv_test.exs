defmodule Bitcom.WeatherSVTest do
  ...
  test "must create a weather object", ctx do
    res = %FBAgent.Cell{script: ctx.script, params: [0x01, "{\"t\":25.22,\"h\":83,\"p\":1009,\"r\":0.25,\"c\":90,\"ws\":2.6,\"wd\":250}", "1HTrdRwefwnTHFGK9Yu2kM23YQb6oETMVu", "1561512601"]}
  ...
  test "must extend a given context", ctx do
    res = %FBAgent.Cell{script: ctx.script, params: [0x01, "{\"t\":25.22,\"h\":83,\"p\":1009,\"r\":0.25,\"c\":90,\"ws\":2.6,\"wd\":250}", "1HTrdRwefwnTHFGK9Yu2kM23YQb6oETMVu", "1561512601"]}
    |> FBAgent.Cell.exec!(ctx.vm, context: %{foo: "bar"})
    assert res["channel"] == "1HTrdRwefwnTHFGK9Yu2kM23YQb6oETMVu"
    assert res["foo"] == "bar"
  end
