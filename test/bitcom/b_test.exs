defmodule Bitcom.BTest do
  ...
  test "must create a file object", ctx do
    file = %FBAgent.Cell{script: ctx.script, params: ["hello world", "text/plain", "utf8", "test.txt"]}
  ...
  test "must default text files to utf8 encoding", ctx do
    file = %FBAgent.Cell{script: ctx.script, params: ["hello world", "text/plain", nil, "test.txt"]}
    |> FBAgent.Cell.exec!(ctx.vm)
    assert file["encoding"] == "utf8"
  end

  test "wont default non text files to utf8 encoding", ctx do
    file = %FBAgent.Cell{script: ctx.script, params: ["hello world", "image/png", nil, "test.png"]}
    |> FBAgent.Cell.exec!(ctx.vm)
    assert file["encoding"] != "utf8"
  end

  test "must handle missing attributes", ctx do
    file = %FBAgent.Cell{script: ctx.script, params: ["hello world", nil, nil, ""]}
    |> FBAgent.Cell.exec!(ctx.vm)
    assert file == %{"data" => "hello world"}
  end
  ...
  test "must handle binary data", ctx do
    ...
    file = %FBAgent.Cell{script: ctx.script, params: [bindata, "application/octet-stream", "binary", "test.bin"]}
    |> FBAgent.Cell.exec!(ctx.vm)
